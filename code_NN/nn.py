...
class NeuralNetwork:
    ...
    def fit(self, X, Y, eta = 0.01, iterations = 1000, SGD = True, mini_batch_size = 1):
        ...
        while iterations > 0:
            ...
            #send smaller portioned minibatches to be worked on
    ...
    def _shuffle(X, Y):
        ...
        :param X: user input samples
        :param Y: user input samples's correct classifications
        :return: shuffled X and shuffled Y with same samples still aligned
        """
        assert len(X) == len(Y)
    ...
    def _gradient_descent(self,X,Y,eta):
        ...
        Neural Network Gradient Descent:
            Feed in the data,
            Backpropogate to calculate how far off are weights are from a good answer (a valley in the function)
            Then update those weights to be less far off (less error)
        :param X: sample minibatch
        :param Y: minibatch Correct labels
    ...
    def _calc_last_layer(self,Y,N):
        """
        Calculate the start of Back propogation function
        :param Y: correct labels for each sample (N samples by k labels
    ...
    def _ff(self,in_X):
        """
        forward feeding function, this updates all the W,S, and X matrices on each layer (besides input layer)
        :param in_X: the input data, with bias column already added (N by d)
        :return:the output classifications k x d-1
        """
    ...
    def _curr_err(self,Y,N):
        """
        This is just if you want to track how much error your NN has at each iteration
        :param Y: Correct labels for each sample (N samples by k labels)
    ...
    def _back_propogate(self,N):
        """
        Back propogate, update all the deltas, and make every G (Error gradient matrix)
        We will be using the Gs to update our W matrices for each layer
        :param N: number of samples
        :return: None
        """
        # last layer's index, L
    ...
    def _update_weights(self,eta):
        """
        Use the error gradients, G, to make our Ws have less error (gradient descent)
        :param eta: how big a step to take in our gradient descent
        :return: None
        """
        # last layer's index, L
        L = len(self.layers) - 1
